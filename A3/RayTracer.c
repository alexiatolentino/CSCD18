...
void rtShade(struct object3D *obj, struct point3D *p, struct point3D *n, struct ray3D *ray, int depth, double a, double b, struct colourRGB *col)
{
  ...
  while(lightsource != NULL){
    ...
    if (lambda > 0 && lambda < 1){
      // Ambient term
      local.R = obj->alpha*obj->alb.ra;
      local.G = obj->alpha*obj->alb.ra;
      local.B = obj->alpha*obj->alb.ra;
    }
    else{
      // Use Phong model to determine local components
      phong(obj, p, n, &ray_ls, ray, &ambient, &diffuse, &specular, 1);
      local.R = obj->alpha* R * (ambient + diffuse.R) + specular.R;
      local.G = obj->alpha* G * (ambient + diffuse.G) + specular.G;
      local.B = obj->alpha* B * (ambient + diffuse.B) + specular.B;
    }
...
void forwardPass(int sx)  {
  ...
  while(obj!=NULL){
    ...
    if (obj->isLightSource) {
      ...
      struct point3D *p0 = (struct point3D *)calloc(1, sizeof(struct point3D));

      // Set up starting point
      p0->px = -1+2*((float)rand())/RAND_MAX;
      p0->py = -1+2*((float)rand())/RAND_MAX;
      p0->pz = 0;
      p0->pw = 1;
      matVecMult(obj->T, p0);
